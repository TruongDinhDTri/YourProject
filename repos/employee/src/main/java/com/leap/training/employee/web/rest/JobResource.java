...
public class JobResource {
     ...
     *         body the new job, or with status {@code 400 (Bad Request)} if the job
     *         has already an ID.
     ...
     *         or with status {@code 500 (Internal Server Error)} if the job
     *         couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/jobs/{jobId}")
    public ResponseEntity<Job> updateJob(@PathVariable(value = "jobId", required = false) final String jobId,
            @RequestBody Job job)
    ...
    /**
     * {@code PATCH  /jobs/:jobId} : Partial updates given fields of an existing
     * job, field will ignore if it is null
     *
     * @param jobId the id of the job to save.
     * @param job   the job to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
    ...
    public ResponseEntity<Job> partialUpdateJob(@PathVariable(value = "jobId", required = false) final String jobId,
            @RequestBody Job job)
            throws URISyntaxException {
        ...
        return ResponseUtil.wrapOrNotFound(result,
                HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, job.getJobId()));
    }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the job, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/jobs/{id}")
